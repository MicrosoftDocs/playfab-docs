...
* Create virtual currencies - Make different types of virtual money that players can use in your game. For example, you can have a "soft" currency that players earn by playing the game, and a "hard" currency that players can buy with real money.
...
* Search for items: Use PlayFab Economy to search for items in your catalog. Search makes it easy to create special in-game views and find what you're looking for.
...
**A:** If you have a live title with existing player data, don't attempt to migrate users today. Although the API and Game Manager surface are similar, migration, especially with live player data, requires diligent engineering.
...
**A:** V2 is a complete rearchitecture from V1, and there are many important differences. At a high level, the Catalog changed from being a single document to individual versioned items, all of which are in their own document with their own metadata. Catalog (V2) supports increased scale for in-game catalogs and intelligent [Search](catalog/search.md) for creating in-game channels of content. V2 requires a different pattern of interaction with the catalog compared to V1. Inventory PlayFab [Inventory](inventory/index.md) is rearchitected to better handle higher amounts of throughput, higher usage calling patterns, and allow for more stable (and idempotent) transactions. Inventory now natively supports [Collections](inventory/collections.md) and [Stacks](inventory/stacks.md). [Stores](catalog/stores.md) and sales are also handled differently in V2.
...
**A:** No, Economy V2 doesn't have full parity with all of the features PlayFab supports in V1 (Legacy). Some of these features never received the adoption we hoped for in Playfab. Others were lacking the depth and surface area needed to really help developers implement them in their games. Some we plan to deliver in future updates. Specifically today Economy V2 doesn't support PlayFab style Coupons, Limited Items, Recharge Item Rates, Drop Tables, or Store Support with Segments. However, some V1 patterns can be replicated using V2 building blocks—for example, [fixed-contents containers can be implemented using bundles and virtual currency](tutorials/containers-with-fixed-contents.md), and item conversion (crafting) can be achieved using the [ExecuteInventoryOperations](/rest/api/playfab/economy/inventory/execute-inventory-operations) API.
...
**A:** The goal of Economy is to make enabling cross platform entitlements easy for titles to manage, and also make it easy when that sharing isn't allowed. Use the features to link a single virtual catalog item to multiple different platform entitlements to ease item management and enable 'Buy Once Play Anywhere scenarios.' You can also, instead, silo virtual currencies, and have multiple separate player inventories, to isolate purchases to comply with different platform obligations. Although the platform features help this way, they don't police or enforce platform policies. You need to implement platform policies for your title.

**Q: Does the platform help abstract away the storefronts of multiple platforms behind a single API?**

**A:** Yes. Set up Economy V2 Bundles to be linkable to items on multiple different platforms. Use platforms to consolidate receipt validation and redemption scenarios. Then use the APIs universally to populate your in-game store, regardless of what the platform is and with little platform specific code post-authorization.
