...
Draft items are created using the `CreateDraftItem` and haven't been published. These items are available to only the Item Creator, [Catalog Admins](../../economy-v2/settings.md#catalog-admins), and [Title Entities](../../../live-service-management/game-configuration/entities/index.md#title). Both `GetDraftItem` and `UpdateDraftItem` can be called on these items.
