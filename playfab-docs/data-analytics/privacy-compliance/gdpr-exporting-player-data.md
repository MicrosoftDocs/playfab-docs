...
| /CustomFiles/MasterPlayerFiles/* | Contains the custom [Entity Files](../../live-service-management/game-configuration/entities/entity-files.md) associated with the player's `master_player_account` entity.|
| /CustomFiles/TitlePlayerFiles/&lt;TitlePlayerAccountId&gt;/* | <p>Contains the custom [Entity Files](../../live-service-management/game-configuration/entities/entity-files.md) associated with the player's `title_player_account` entities.</p><p>The files are organized into one subdirectory per title that the player participates in. The name of each subdirectory is the player's `title_player_account` entity ID in the title.</p> |
