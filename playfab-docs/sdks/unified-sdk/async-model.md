---
title: Making async calls in the PlayFab Unified SDK
author: amccalib
description: Making async calls in the PlayFab Unified SDK
ms.author: jeffshi
ms.date: 03/03/2023
ms.topic: article
...
# Making async calls in the PlayFab Unified SDK
...
The PlayFab Unified SDK exposes an asynchronous C API that gives developers direct thread control when making an asynchronous API call.
...
**workDispatchMode** determines the dispatch mode for the thread that handles the async work. **completionDispatchMode** determines the dispatch mode for the thread that handles the completion of the async operation.
