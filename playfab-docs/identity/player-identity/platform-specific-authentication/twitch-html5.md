...
This tutorial requires a web server, in order to follow the instructions. If you don't have a registered remote web server yet, follow our [Running an HTTP server for testing](running-an-http-server-for-testing.md) tutorial, for information on how to run a local web server.
