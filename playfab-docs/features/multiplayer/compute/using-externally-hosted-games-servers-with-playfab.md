...
- [RegisterGame](https://api.playfab.com/documentation/Server/method/RegisterGame) - Notifies PlayFab about a new server instance.
- [DeregisterGame](https://api.playfab.com/documentation/Server/method/DeregisterGame) - Notifies PlayFab that a server instance is going away.
- [RefreshGameServerInstanceHeartbeat](https://api.playfab.com/documentation/Server/method/RefreshGameServerInstanceHeartbeat) - Lets PlayFab know that the server is still running.

In addition, you should be aware of the following API methods:

- [GetCurrentGames](https://api.playfab.com/documentation/Client/method/GetCurrentGames) - Gets the listing of all the active sessions running.
- [Matchmake](https://api.playfab.com/documentation/Client/method/Matchmake) - Finds an open slot in an active server matching the search conditions, if one is available.
- [RedeemMatchmakerTicket](https://api.playfab.com/documentation/server/method/RedeemMatchmakerTicket) - Used by the server to verify that the player’s session is valid, that they joined the server from a legitimate matchmaking call, and provides the server with basic account information for the player.
- [NotifyMatchmakerPlayerLeft](https://api.playfab.com/documentation/server/method/NotifyMatchmakerPlayerLeft) - Used by the server to tell PlayFab that the player has left the server, opening a slot for matchmaking.
- [SetGameServerInstanceState](https://api.playfab.com/documentation/server/method/SetGameServerInstanceState) - Provides a way to close a server to matchmaking.
- [SetGameServerInstanceTags](https://api.playfab.com/documentation/server/method/SetGameServerInstanceTags) - Adds tags to server instances that can be used for filtering in matchmaking.
- [SetGameServerInstanceData](https://api.playfab.com/documentation/server/method/SetGameServerInstanceData) - Sets custom data on a server instance which is returned when querying the list of running servers.
...
Once the game server is up and running, matchmaking works exactly as it does for servers hosted by PlayFab. Calls to [GetCurrentGames](https://api.playfab.com/documentation/Client/method/GetCurrentGames) will return the listing of game servers running, and [Matchmake](https://api.playfab.com/documentation/Client/method/Matchmake) will return the information for the server with an available slot which is the best fit to the player’s search parameters. The player passes the matchmaker ticket returned by Matchmake to that server, which uses [RedeemMatchmakerTicket](https://api.playfab.com/documentation/Server/method/RedeemMatchmakerTicket) to validate that the user has a good login, was sent to the server as a result of a valid matchmaking call, and to retrieve the user’s information. That call also tells the PlayFab matchmaker that the player has joined the server, occupying the slot (which is “reserved” for two minutes after the call to Matchmake). When the player has left the server, the occupied slot is then freed up by calling [NotifyMatchmakerPlayerLeft](https://api.playfab.com/documentation/Server/method/NotifyMatchmakerPlayerLeft).

Now, since the PlayFab Game Wrangler has no direct access to your servers, it’s important for them to regularly let the service know that they’re still running. Regardless of whether the server has available slots or not, make sure to call [text](https://api.playfab.com/documentation/Server/method/RefreshGameServerInstanceHeartbeat) once every minute, to ensure the server is maintained in the matchmaker’s memory.
