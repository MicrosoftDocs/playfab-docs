...
Filtering remote IP addresses for UDP connectivity via domain name resolution is possible but strongly discouraged. Currently, all connections to transparent cloud relay servers resolve names of the form *\*.cloudapp.azure.com*, but this pattern is subject to change without notice. Restricting UDP connectivity to only IP addresses resolved by names will also prevent direct peer-to-peer connection.
