...
- In the extracted folder, you will find a **MultiplayerSettings.json** file, open it in a text editor (such as [Visual Studio Code](https://code.visualstudio.com/download)) and update the following properties:  
  ...
  - **StartGameCommand**: The full path to the game server executable within the container (that includes the mount path above). Assuming the executable name is **mygame.exe**, a sample path would be `C:\\Assets\\mygame.exe`. There is a different StartGameCommand for Process vs. Container. The container StartGameCommand should be an absolute path to a resource in the container or asset folder. The process StartGameCommand should be a relative path where the working directory will be the first asset specified.
  - **PortMappingsList**: These are the ports that will be available to your game while running. NodePort is the port that will be opened on your machine, GamePort.Number is the port that your game server needs to bind to when running in a container. Update the GamePort section to match the protocol and port at which your game server is listening for clients. If your game needs multiple ports, copy paste the existing port configuration and increment NodePort (along updating the GamePort settings). When running as a process, GamePort.Number is ignored, your process should bind to NodePort. To handle both cases, set the ports to the same value or check the gsdk config at runtime for the value with key GamePort.Name which will always return the correct port to bind against.
  
- There are additional fields in that file you may want to edit:
  - **ResourceLimits** (optional): If specified, docker will limit CPU/memory usage. Warning: If your server goes over the allowed memory, it will be killed. ResourceLimits can only be specified in container mode.
  ...
  - **OutputFolder** (optional):  Path to a drive or folder where the outputs and config files will be generated. Ensure there is sufficient space available since the game server will be extracted under this path. If not specified, the agent folder will be used
  - **MountPath**: The path within the container that the asset should be mounted at. This field does not need to be specified when running in process mode. We recommend using the sample value - `C:\\Assets` (note that backslashes need to be escaped for JSON formatting).
  - **AgentListeningPort**: This is the port that the agent will bind to for talking to the game server. Any open port will work, if you have another process binding to 56001 you will need to change this value (or kill the other process).
...
Once your game executable is running and **MockVmAgent** output prints **CurrentGameState - Active**, you should be able to connect to your game using **ipaddress 127.0.0.1** and the port (NodePort) at which your game server is listening on.

The **MockVmAgent** requests the game server to move from standby to active and then from active to terminated (via GSDK) after a configurable number of heartbeats. This behavior can be tuned by updating **NumHeartBeatsForActivateResponse** and **NumHeartBeatsForTerminateResponse** in the **MultiplayerSettings.json** file.
...
If the game server was run within a container, there might be an additional level of directory hierarchy to sift through. The GSDK itself emits some logging for debugging purposes. These logs are also located within the GameLogs folder.  
...
1. Containers may not be stopped at the end of the debugging. Please run the commands below in a powershell (as Administrator). Note that these will stop and remove all containers, including ones not started by the MockVmAgent.
