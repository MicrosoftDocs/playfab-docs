...
* **Telemetry Events** - Telemetry Events are metered based on the total number of Telemetry Event API call that are processed by PlayFab.* For more information, see [PlayStream Events - Write Telemetry Events](xref:titleid.playfabapi.com.events.playstreamevents.writetelemetryevents). For limits on Telemetry Events, see [Getting Started with Insights](../../Insights/insights/quickstart.md).
