---
author: jasonsandlin
title: "PFAuthenticationLoginWithGameCenterGetResult"
description: "Get the result from a PFAuthenticationLoginWithGameCenterAsync call. The PFEntityHandle will always be returned, but the additional info in the PFAuthenticationLoginResult is only returned if a buffer is provided."
ms.author: jasonsa
ms.topic: reference
...
If the PFAuthenticationLoginWithGameCenterAsync call fails, entityHandle with be null. Otherwise, the handle must be closed with PFEntityCloseHandle when it is no longer needed. If returned, 'result' is a pointer within 'buffer' and does not need to be freed separately.
