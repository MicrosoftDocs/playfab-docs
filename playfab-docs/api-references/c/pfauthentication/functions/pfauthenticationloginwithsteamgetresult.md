---
author: jasonsandlin
title: "PFAuthenticationLoginWithSteamGetResult"
description: "Get the result from a PFAuthenticationLoginWithSteamAsync call. The PFEntityHandle will always be returned, but the additional info in the PFAuthenticationLoginResult is only returned if a buffer is provided."
ms.author: jasonsa
ms.topic: reference
...
```cpp
HRESULT PFAuthenticationLoginWithSteamGetResult(  
    XAsyncBlock* async,  
...
If the PFAuthenticationLoginWithSteamAsync call fails, entityHandle with be null. Otherwise, the handle must be closed with PFEntityCloseHandle when it is no longer needed. If returned, 'result' is a pointer within 'buffer' and does not need to be freed separately.
